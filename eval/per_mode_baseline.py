"""Fixed-mode KPI baseline using the standalone synthetic simulation."""
...
from modeling import DriveMode, StandaloneSimulation

MODES = (DriveMode.EV, DriveMode.ECO, DriveMode.NORMAL, DriveMode.PWR)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate per-mode KPI baselines (synthetic cycle).")
    ...
    parser.add_argument("--quiet", action="store_true", help="Suppress simulator logs.")
    return parser.parse_args()


def run_fixed_mode(mode: DriveMode, run_dir: Path, quiet: bool) -> pd.DataFrame:
    run_dir.mkdir(parents=True, exist_ok=True)
    csv_path = run_dir / f"synthetic_{mode.value.lower()}_kpis.csv"
    sim = StandaloneSimulation(init_drive_mode=mode, csv_path=str(csv_path))
...
def compute_kpis(df: pd.DataFrame) -> dict[str, float | int | str | bool]:
    dt = timestep_s(df)
    ...
    dist_km = float(np.sum(speed_ms * dt) / 1000.0)
...
def save_fuel_bar(kpis: pd.DataFrame, figure_dir: Path) -> Path:
    ...
    vals = [float(kpis[kpis["mode"] == l]["total_fuel_g"].iloc[0]) for l in labels]
    fig, ax = plt.subplots(figsize=(7, 5))
    ax.bar(labels, vals, color=["#4e79a7", "#f28e2b", "#59a14f", "#e15759"], alpha=0.85)
    ax.set_title("Fixed-Mode Fuel Baseline (Synthetic Cycle)")
...
def save_soc_traces(all_runs: list[pd.DataFrame], figure_dir: Path) -> Path:
    ...
    for mode, df in zip(MODES, all_runs):
    ...
    ax.set_title("Fixed-Mode SOC Trace (Synthetic)")
    ax.set_ylabel("SOC (%)")
    ax.set_xlabel("Time (s)")
    ax.legend(title="Mode", ncol=4, loc="upper right")
    ax.grid(alpha=0.25)
    fig.tight_layout()
...
def main() -> None:
    ...
    rows: list[dict] = []
    for mode in MODES:
        df = run_fixed_mode(mode, run_dir, args.quiet)
        all_runs.append(df)
        kpi = compute_kpis(df)
        rows.append(kpi)
        print(f"  {mode.value:6s} -> {len(df):4d} steps  fuel={kpi['total_fuel_g']:.1f}g")
    ...
    normal = kpis[kpis["mode"] == "NORMAL"].iloc[0]
    print(f"KPI table   -> {output_path}")
    print(f"Fuel bar    -> {fuel_bar_path}")
    print(f"SOC traces  -> {soc_traces_path}")
    print(f"NORMAL fuel: {normal['total_fuel_g']:.3f} g")
